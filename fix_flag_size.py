...
# 2. Flag Parameters - REDUCED SIZE
# Original was 12x8. Let's try 8x5.
flag_width = 8.0
flag_height = 5.0
segments_x = 30
segments_z = 20
...
for j in range(segments_z + 1):
    ...
    # Simpler way to get x_hoist_local:
    # At top (z_local=0), x_hoist_local=0
    # For every 1 unit down in z_local, x_hoist_local changes by -tan_tilt? 
    # Actually, as z_local goes negative (down), the pole goes towards post_x (forward relative to top).
    # top_pos[0] is post_x + sin(-15)*8 (approx -19 - 2 = -21)
    # base_x is -19.
    # So as we go down, X increases from -21 towards -19.
    # z_local is from 0 to -5.
    # x_hoist_local = -z_local * tan_tilt? 
    # Let's re-verify: if z_local = -post_length_z (approx -7.7), 
    # then x_hoist_local = -(-7.7) * tan(-15) = 7.7 * (-0.26) = -2.0.
    # top_pos[0] + (-2) = -21 + (-2) = -23. WRONG.
    # tan_tilt = math.tan(math.radians(-15)) is approx -0.26.
    # We want x to INCREASE as we go DOWN.
    # So x_hoist_local = z_local * tan_tilt.
    # If z_local = -5, x_hoist_local = -5 * -0.26 = +1.3. 
    # -21 + 1.3 = -19.7. Correct, it's moving towards -19.
...
print("Created resized horizontal Dutch flag (8m x 5m) pointing forward.")
"""
    try:
